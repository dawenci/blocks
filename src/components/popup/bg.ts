...
function makeTopData(rectWidth: number, rectHeight: number, arrowHeight: number, arrowOffset: number, radius: number) {
...
function makeLeftData(rectWidth: number, rectHeight: number, arrowWidth: number, arrowOffset: number, radius: number) {
...
function makeRightData(rectWidth: number, rectHeight: number, arrowWidth: number, arrowOffset: number, radius: number) {
...
export function updateBg({ $svg, width, height, arrowSize, lineWidth, radius, stroke, fill, origin }: MakeOptions) {
  const $path = $svg.children[0]

  let arrowOffset = 0
  // 取四分之一处和固定值中比较靠近边沿的那个
  ...
  // 需要设置偏移线宽的二分之一，因为描边是往线条中心的两侧扩张的，如果不设置，线条会被裁剪一半
  switch (origin) {
    case 'top-start':
    case 'top-end':
    case 'top-center': {
