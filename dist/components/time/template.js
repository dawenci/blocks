...
export const _template = makeTemplate(`
<div id="layout">
...
range(0, 23).forEach(n => {
    $hours.appendChild(_$item.cloneNode(true)).textContent = n < 10 ? '0' + n : String(n);
});
$hours.appendChild(_$bot.cloneNode(true));
const $minutes = _$dom.querySelector('#minutes');
range(0, 59).forEach(n => {
    $minutes.appendChild(_$item.cloneNode(true)).textContent = n < 10 ? '0' + n : String(n);
});
$minutes.appendChild(_$bot.cloneNode(true));
const $seconds = _$dom.querySelector('#seconds');
range(0, 59).forEach(n => {
    $seconds.appendChild(_$item.cloneNode(true)).textContent = n < 10 ? '0' + n : String(n);
});
