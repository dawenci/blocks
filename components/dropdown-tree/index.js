...
import BlocksPopupTree from '../popup-tree/index.js'
...
const ATTRS = BlocksPopupTree.observedAttributes.concat(['trigger-mode'])

export default class BlocksDropDownTree extends HTMLElement {
  ...
  constructor() {
    ...
    this.$popup = document.createElement('bl-popup-tree')
    this.$popup.anchor = () => this.$slot.assignedElements()?.[0] ?? this

    forEach(this.attributes, (attr) => {
      if (BlocksPopupTree.observedAttributes.includes(attr.name)) {
    ...
    this.$popup.addEventListener('change', event => {
      ...
      if ($trigger && $trigger.acceptValue) {
        if (this.$tree.checkable === 'multiple') {
          ...
          $trigger.acceptValue(value.map(item => ({ value: this.$tree.internalKeyMethod(item), label: this.$tree.internalLabelMethod(item) })))
          this.labelField
        }
        else {
          const value = this.$tree.checkedData[0] ?? {}
          $trigger.acceptValue({
            value: this.$tree.internalKeyMethod(value),
            label: this.$tree.internalLabelMethod(value),
          })
        }
      }
  ...
  attributeChangedCallback(attrName, oldValue, newValue) {
    if (BlocksPopupTree.observedAttributes.includes(attrName)) {
