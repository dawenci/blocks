...
/* 表头区视口 */
.VGridHeader .VGridHeaderViewport {
  flex: 0 0 auto;
  overflow: hidden;
  // 表头区内容排版容器
  .VGridHeaderCanvas {
...
/* 隔行换色 */
.VGridBody_row.VGridBody_row-odd {
  background-color: #fafafa;
}

/* 激活的行 */
.VGridBody_row.VGridBody_row-active {
...
:host([border]) .VGrid .VGrid_cell {
  border-right: 1px solid $--border-color-lighter;
}
/* 最后一列不要右描边 */
/* 注：表头有合并格子比较复杂，不在这里处理，最后一列的右描边使用 after 盖住 */
:host([border]) .VGrid .VGridBody_row > .VGrid_cell:last-child,
:host([border]) .VGrid .VGridFooterCanvas > .VGrid_cell:last-child {
  border-right: 0 none;
}
...
class RowColumn {
  constructor(options = {}) {
    ...
    this.children = (options.children ?? []).map(child => child instanceof RowColumn ? child : new RowColumn(child))
  }
}
...
getFlattenColumns(pred) {
  ...
  this.store.columns.forEach(child => flat(child, null))
  return columns
}
...
shouldShowFixedColumns() {
  ...
  if (this.store.columns.every(column => column.fixedLeft || column.fixedRight)) {
    return false
  }
...
// @Provide()
hasFixedLeft() {
  return this.store.columns.some(column => !!column.fixedLeft)
}

// 是否有右固定列
// @Provide()
hasFixedRight() {
  return this.store.columns.some(column => !!column.fixedRight)
}
...
fixedLeftWidth() {
  return this.getFlattenFixedColumns('left')
    .reduce((acc, column) => acc + column.columnWidth, 0)
}
...
// 首次绑定数据时，检查是否需要折叠所有结点
// @Watch('isDataBound')
_onDataBoundChange() {
  this.$emit('data-bound')
}
...
layout(canvasWidth) {
  ...
  // 没有未分配的尺寸，无需调整
  if (remainingWidth === 0) {
    this.$emit('layout', store)
    return
  }

  // 未分配的尺寸大于 0，需要将这些尺寸加在各个 column 上
  if (remainingWidth > 0) {
    this._expandColumns(remainingWidth, store.columns)
    this.$emit('layout', store)
    return
  }

  // 未分配的尺寸小于 0，需要将这些收缩量分配到 column 上
  this._shrinkColumns(-remainingWidth, store.columns)
...
_getExpandSize(column) {
  const size = column.maxWidth - column.columnWidth
...
_getShrinkSize(column) {
  const size = column.columnWidth - column.minWidth
  if (size > 0) return size
  return 0
}
