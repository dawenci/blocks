...
export class BlocksSplitter extends HTMLElement {
  ...
  getPaneSize($pane) {
    ...
    size = Math.max(size, $pane.min, this.panes[0] === $pane ? 0 : this.handleSize)
...
export class BlocksSplitterPane extends HTMLElement {
  ...
  constructor() {
    ...
    const onmove = e => {
      ...
      // 刷新
      const axis = this.$splitter.direction === 'row' ? 'x' : 'y'
  ...
  updateStyle() {
    const isFirstPane = this.$splitter.panes[0] === this
    ...
    this.$content.style[sizeProp] = isFirstPane
    ...
    if (!isFirstPane) {
      this.style['padding-' + posProp] = this.$splitter.handleSize + 'px'
    }

    // 设置 handle
    this.$handle.style.width = this.$handle.style.height = ''
    this.$handle.style[sizeProp] = isFirstPane ? '0' : this.$splitter.handleSize + 'px'
  }
