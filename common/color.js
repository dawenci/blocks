...
export const parse = (() => {
  ...
  function parseRgba(input) {
    ...
    const rgba = valueStr.split(/\s*,\s*/).map(str => parseFloat(str, 10))
    if (isRgba && rgba.length !== 4) return null
    if (!isRgba && rgba.length !== 3) return null
    const [r, g, b, a] = rgba
    if (r < 0 || r > 255) return null
    if (g < 0 || g > 255) return null
    if (b < 0 || b > 255) return null
    if (isRgba && (a < 0 || a > 1)) return null
  ...
  function parseHsl(input) {
    ...
    const end = input.length - 1
    const valueStr = input.slice(start, end)
    const hsla = valueStr.split(/\s*,\s*/).map(str => parseFloat(str, 10))
